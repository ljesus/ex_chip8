defmodule ExChip8.Interpreter do
  alias __MODULE__
  ...
  def new(rom_path) do
    %Interpreter{
      memory: ExChip8.Memory.new(rom_path),
      pc: 0x200,
      running: false,
  ...
  def cycle(%Interpreter{memory: memory, pc: pc} = interpreter) do
    try do
      # All instructions are 2 bytes long and are stored most-significant-byte first.
      opcode = binary_part(memory.data, pc, 2)
      new_interpreter = execute(%{interpreter | pc: pc + 2}, opcode)
      {:ok, %{interpreter: new_interpreter, opcode: opcode}}
    rescue
      err ->
        IO.inspect(err)
        {:error, :stop}
